...
class EventSplitterTrainingService(EventSplitterServiceBase, BaseTrainingService):
    step_key = "train"
    ...
    def execute(self) -> GraphLightningModule:
        ...
        model = self.objective_adapter.build_model(
            ...
            compile_cfg=None,
            context="train_event_splitter",
        )
        model = self.compile_model(model, compile_cfg=cfg.get("compile"), context="train_event_splitter")
        ...
        if not train_provider.include_targets or not val_provider.include_targets:
            raise RuntimeError("EventSplitterGraphLoader must run in train mode for training/validation.")
