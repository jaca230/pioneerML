...
from pioneerml.pipelines.training.event_splitting.objective import EventSplitterObjectiveAdapter


class EventSplitterServiceBase(BasePipelineService):
    def __init__(
        self,
        *,
        ...
        if not isinstance(factory, EventSplitterGraphLoaderFactory):
            raise RuntimeError("Dataset is missing EventSplitterGraphLoaderFactory.")
        self.loader_factory = factory
        self.objective_adapter = EventSplitterObjectiveAdapter()
    ...
    def _resolve_loader_params(self, cfg: dict, *, purpose: str, forced_batch_size: int | None = None) -> dict:
        ...
        if forced_batch_size is not None:
            ...
            merged.setdefault("batch_size", int(cfg.get("batch_size", 8)))
