...
class EndpointRegressorInferenceRunService(EndpointRegressorInferenceServiceBase, BaseInferenceService):
    step_key = "inference"
        ...
        *,
        ...
        with torch.no_grad():
            for file_idx, src_file in enumerate(validated_files):
                ...
                loader = EndpointRegressorGraphLoader(
                ...
                for batch in loader:
                    ...
                    u = batch.u.to(device, non_blocking=(device.type == "cuda"))
                    ...
                    preds = scripted(x, edge_index, edge_attr, b, u, group_probs, splitter_probs)
