...
from pioneerml.common.integration.pytorch.models.architectures.graph.transformer.classifiers.group_splitter import (
    GroupSplitter,
)
...
class GroupSplitterObjectiveAdapter(BaseObjectiveAdapter):
    ...
    def suggest_model_params(self, *, trial: optuna.Trial, model_search_cfg: dict | None = None) -> dict:
        ...
        hidden_low, hidden_high, _ = suggest_range(cfg, "hidden", default_low=64, default_high=256)
        ...
        return {
            "node_dim": int(GroupSplitterGraphLoader.NODE_FEATURE_DIM),
            "graph_dim": int(GroupSplitterGraphLoader.NUM_CLASSES),
            "edge_dim": int(GroupSplitterGraphLoader.EDGE_FEATURE_DIM),
            "hidden": int(hidden),
            "heads": int(heads),
    ...
    def default_model_cfg(self, model_cfg: dict | None = None) -> dict:
        ...
        cfg.setdefault("node_dim", int(GroupSplitterGraphLoader.NODE_FEATURE_DIM))
        cfg.setdefault("graph_dim", int(GroupSplitterGraphLoader.NUM_CLASSES))
        cfg.setdefault("edge_dim", int(GroupSplitterGraphLoader.EDGE_FEATURE_DIM))
    ...
    def build_model(self, *, model_cfg: dict, compile_cfg: dict | None, context: str):
        ...
        return GroupSplitter(
            ...
            edge_dim=int(cfg.get("edge_dim", int(GroupSplitterGraphLoader.EDGE_FEATURE_DIM))),
            hidden=hidden,
            heads=heads,
            layers=int(cfg.get("layers", 3)),
            dropout=float(cfg.get("dropout", 0.1)),
            num_classes=int(GroupSplitterGraphLoader.NUM_CLASSES),
        )
