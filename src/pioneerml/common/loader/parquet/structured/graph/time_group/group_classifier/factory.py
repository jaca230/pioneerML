...
class GroupClassifierGraphLoaderFactory:
    def __init__(self, *, parquet_paths: list[str] | None = None, parquet_inputs: ParquetInputSet | None = None) -> None:
        self.parquet_inputs = (
            parquet_inputs
            if parquet_inputs is not None
            else ParquetInputSet(main_paths=[str(p) for p in (parquet_paths or [])])
        )
