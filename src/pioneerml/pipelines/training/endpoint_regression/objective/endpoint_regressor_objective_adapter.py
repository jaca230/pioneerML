...
from pioneerml.common.models.graph.transformer.regressors import EndpointRegressor
...
class EndpointRegressorObjectiveAdapter(BaseObjectiveAdapter):
    @staticmethod
    ...
    def suggest_model_params(self, *, trial: optuna.Trial, model_search_cfg: dict | None = None) -> dict:
        ...
        return {
            "node_dim": int(EndpointRegressionGraphLoader.NODE_FEATURE_DIM),
            "graph_dim": int(EndpointRegressionGraphLoader.GRAPH_FEATURE_DIM) - 1,
            "splitter_prob_dimension": 0,
            "edge_dim": int(EndpointRegressionGraphLoader.EDGE_FEATURE_DIM),
            "hidden": int(hidden),
            "heads": int(heads),
            ...
            "output_dim": int(EndpointRegressionGraphLoader.TARGET_DIM),
        }
    ...
    def default_model_cfg(self, model_cfg: dict | None = None) -> dict:
        ...
        cfg.setdefault("node_dim", int(EndpointRegressionGraphLoader.NODE_FEATURE_DIM))
        cfg.setdefault("graph_dim", int(EndpointRegressionGraphLoader.GRAPH_FEATURE_DIM) - 1)
        cfg.setdefault("splitter_prob_dimension", 0)
        cfg.setdefault("edge_dim", int(EndpointRegressionGraphLoader.EDGE_FEATURE_DIM))
        cfg.setdefault("hidden", 192)
        cfg.setdefault("heads", 4)
        cfg.setdefault("layers", 3)
        cfg.setdefault("dropout", 0.1)
        cfg.setdefault("output_dim", int(EndpointRegressionGraphLoader.TARGET_DIM))
        return cfg
    ...
    def build_model(self, *, model_cfg: dict, compile_cfg: dict | None, context: str):
        ...
        return EndpointRegressor(
            ...
            graph_dim=int(cfg["graph_dim"]),
            splitter_prob_dimension=int(cfg.get("splitter_prob_dimension", 0)),
            edge_dim=int(cfg.get("edge_dim", int(EndpointRegressionGraphLoader.EDGE_FEATURE_DIM))),
            hidden=hidden,
            heads=heads,
