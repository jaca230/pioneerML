...
class PionStopGraphLoader(TimeGroupGraphLoader):
    """Chunked graph loader for pion-stop quantile regression."""
    ...
    TARGET_COLUMNS = ("pion_stop_x", "pion_stop_y", "pion_stop_z")
    TARGET_BASE_DIM = 3
    TARGET_DIM = 9
    MIN_RELEVANT_HITS = 1
    ...
    def _compute_relevant_graph_mask(
        ...
        *,
        ...
        if sorted_particle_mask is None:
            raise RuntimeError(
                f"{self.__class__.__name__} requires hits_particle_mask for training_relevant_only=True."
            )
        rel_hits = self._relevant_hit_mask(sorted_particle_mask)
        if rel_hits.size == 0 or not np.any(rel_hits):
            return np.zeros((total_graphs,), dtype=bool)
        counts = np.bincount(sorted_group_ids[rel_hits], minlength=total_graphs).astype(np.int64, copy=False)
