...
def load_group_splitter_dataset_local(
    parquet_paths,
    ...
    adapter = pml.adapters.input.graph.GroupSplitterInputAdapter()
    ...
    group_probs = _DATASET_BUILDER.arrow_to_torch(inputs.group_probs, shape=(num_graphs, 3), dtype=torch.float32)
    ...
    return GroupSplitterDataset(data=data, targets=y_node)
...
def load_group_splitter_dataset(
    ...
    return load_group_splitter_dataset_local(
        parquet_paths,
