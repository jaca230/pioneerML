...
from pioneerml.common.integration.pytorch.model_handle import ModelHandleFactory

from .....resolver import BasePayloadResolver


class ModelHandleBuilderRuntimeStateResolver(BasePayloadResolver):
    ...
    def resolve(
        ...
        *,
        ...
        if selected is None:
            selected = self.step.config_json.get("model_path")
        ...
        runtime_state["model_handle"] = ModelHandleFactory.build(
    ...
    def _resolve_model_type(self, *, payloads: Mapping[str, Any] | None) -> str:
        ...
        raw = self.step.config_json.get("model_type", "script")
    ...
    def _resolve_model_subdir(self) -> str:
        out = str(self.step.config_json.get("model_subdir", "")).strip()
        if not out:
            raise RuntimeError(f"{self.step.__class__.__name__} config missing non-empty 'model_subdir'.")
        return out
