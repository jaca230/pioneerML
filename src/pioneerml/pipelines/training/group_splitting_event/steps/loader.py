...
from pioneerml.common.zenml.materializers import GroupSplitterEventDatasetMaterializer
...
def load_group_splitter_event_dataset_local(
    parquet_paths,
    ...
    adapter = pml.adapters.input.graph.GroupSplitterEventInputAdapter()
    ...
    y_node = _DATASET_BUILDER.arrow_to_torch(targets.y_node, shape=(num_nodes, 3), dtype=torch.float32)
    data.y = y_node

    return GroupSplitterEventDataset(data=data, targets=y_node)


@step(enable_cache=False, output_materializers=GroupSplitterEventDatasetMaterializer)
def load_group_splitter_event_dataset(
    ...
    return load_group_splitter_event_dataset_local(
        parquet_paths,
