...
from pioneerml.common.zenml.materializers import EventSplitterEventDatasetMaterializer
...
def load_event_splitter_event_dataset_local(
    parquet_paths,
    group_probs_parquet_paths=None,
    group_splitter_parquet_paths=None,
    endpoint_parquet_paths=None,
    config_json=None,
    normalize: bool = False,
    normalize_eps: float = 1e-6,
) -> EventSplitterEventDataset:
    adapter = pml.adapters.input.graph.EventSplitterEventInputAdapter()
    extra_paths = {}
    if group_splitter_parquet_paths is not None:
        extra_paths["group_splitter_probs"] = group_splitter_parquet_paths
    if endpoint_parquet_paths is not None:
        extra_paths["endpoint_preds"] = endpoint_parquet_paths
    ...
    bundle = _DATASET_BUILDER.load_training_bundle(
        ...
        extra_parquet_paths_by_key=extra_paths or None,
    )
    ...
    endpoint_preds = _DATASET_BUILDER.arrow_to_torch(inputs.endpoint_preds, shape=(num_groups, 6), dtype=torch.float32)
    ...
    y_edge = _DATASET_BUILDER.arrow_to_torch(targets.y_edge, shape=(num_edges, 1), dtype=torch.float32)
    data.y = y_edge

    return EventSplitterEventDataset(data=data, targets=y_edge)


@step(enable_cache=False, output_materializers=EventSplitterEventDatasetMaterializer)
def load_event_splitter_event_dataset(
    ...
    return load_event_splitter_event_dataset_local(
        parquet_paths,
