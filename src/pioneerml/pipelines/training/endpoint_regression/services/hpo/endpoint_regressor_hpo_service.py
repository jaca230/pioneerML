...
class EndpointRegressorHPOService(EndpointRegressorServiceBase, BaseHPOService):
    step_key = "hpo"
    ...
    def build_hpo_loaders(self, *, cfg: dict, batch_size: int):
        ...
        if not train_provider.include_targets or not val_provider.include_targets:
            raise RuntimeError("EndpointRegressorGraphLoader must run in train mode for HPO.")
