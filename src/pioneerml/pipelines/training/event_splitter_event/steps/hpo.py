...
from pioneerml.pipelines.training.event_splitter_event.steps.train import (
...
def tune_event_splitter_event(
    ...
    defaults = {
        ...
        "direction": "minimize",
        "seed": None,
        "study_name": "event_splitter_event_hpo",
        "storage": None,
        "fallback_dir": None,
    ...
    if "group_prob_dimension" not in base_model_cfg:
        base_model_cfg["group_prob_dimension"] = int(dataset.data.group_probs.shape[-1])
    if "splitter_prob_dimension" not in base_model_cfg:
        base_model_cfg["splitter_prob_dimension"] = int(dataset.data.splitter_probs.shape[-1])
    if "endpoint_dimension" not in base_model_cfg:
        base_model_cfg["endpoint_dimension"] = int(dataset.data.endpoint_preds.shape[-1])
    if "edge_attr_dimension" not in base_model_cfg:
        base_model_cfg["edge_attr_dimension"] = int(dataset.data.edge_attr.shape[-1])
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        hidden_low, hidden_high, _ = _suggest_range(base_model_cfg, "hidden", default_low=96, default_high=320)
        ...
        model = EventSplitter(
            ...
            group_prob_dimension=int(base_model_cfg["group_prob_dimension"]),
            splitter_prob_dimension=int(base_model_cfg["splitter_prob_dimension"]),
            endpoint_dimension=int(base_model_cfg["endpoint_dimension"]),
            edge_attr_dimension=int(base_model_cfg["edge_attr_dimension"]),
            hidden=hidden,
            heads=heads,
            layers=layers,
    ...
    study, storage_used = _optimize(
        ...
        study_name=str(cfg.get("study_name", "event_splitter_event_hpo")),
