...
class EndpointRegressorHPOStep(BaseHPOStep):
    ...
    def objective_context(self) -> str:
        return "tune_endpoint_regressor"

    def study_name_default(self) -> str:
        return "endpoint_regressor_hpo"
    ...
    def build_hpo_loaders(self, *, cfg: dict, batch_size: int):
        ...
        if not train_provider.include_targets or not val_provider.include_targets:
            raise RuntimeError("EndpointRegressionGraphLoader must run in train mode for HPO.")
...
def tune_endpoint_regressor_step(
    ...
    return EndpointRegressorHPOStep(dataset=dataset, pipeline_config=pipeline_config).execute()
