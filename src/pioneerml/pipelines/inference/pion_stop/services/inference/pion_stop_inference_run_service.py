...
class PionStopInferenceRunService(PionStopInferenceServiceBase, BaseInferenceService):
    step_key = "inference"
    ...
    def default_config(self) -> dict:
        return {
            ...
            "streaming_tmp_dir": ".cache/pioneerml/inference/pion_stop",
            "prediction_cuts": {
                "enabled": True,
                "group_prob_threshold": 0.5,
                "splitter_pion_threshold": 0.5,
            },
        }
    ...
    def _compute_prediction_mask(
        *,
        ...
        is_muon = g[:, 1] > float(group_prob_threshold)
        is_mip = g[:, 2] > float(group_prob_threshold)
        ambiguous = is_pion & is_muon & (~is_mip)
        keep = is_pion & (~ambiguous)
        if not torch.any(ambiguous):
            return keep

        has_split_pion = torch.zeros((g.shape[0],), dtype=torch.bool)
        if (
            splitter_probs.ndim == 2
            and splitter_probs.shape[0] == batch_index.shape[0]
            and splitter_probs.shape[1] >= 3
            and splitter_probs.numel() > 0
            and batch_index.numel() > 0
        ):
            split = splitter_probs.to(torch.float32)
            pion_hit = (split.argmax(dim=1) == 0) | (split[:, 0] > float(splitter_pion_threshold))
            if torch.any(pion_hit):
                has_split_pion[batch_index[pion_hit].to(torch.int64)] = True
        return keep | (ambiguous & has_split_pion)
    ...
    def execute(
        ...
        *,
        ...
        splitter_pion_threshold = float(cuts_cfg.get("splitter_pion_threshold", 0.5))
        ...
        if streaming:
            tmp_root = str(cfg.get("streaming_tmp_dir", ".cache/pioneerml/inference/pion_stop"))
                ...
                for batch in loader:
                    ...
                    if use_prediction_cuts:
                        keep_mask = self._compute_prediction_mask(
                            ...
                            splitter_probs=batch.splitter_probs.to(torch.float32).cpu(),
                            batch_index=batch.batch.to(torch.int64).cpu(),
                            group_prob_threshold=group_prob_threshold,
                            splitter_pion_threshold=splitter_pion_threshold,
                        )
                        if keep_mask.shape[0] != preds_cpu.shape[0]:
                            raise RuntimeError(
                                "Prediction-cut mask size mismatch in pion-stop inference: "
