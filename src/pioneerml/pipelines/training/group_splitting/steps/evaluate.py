...
def evaluate_group_splitter(
    ...
    batch_size = int(step_config.get("batch_size", 64)) if step_config else 64
    base_loader = getattr(dataset, "loader", None)
    if not isinstance(base_loader, GroupSplitterGraphLoader):
        raise RuntimeError("Dataset is missing GroupSplitterGraphLoader required for chunked evaluation.")
