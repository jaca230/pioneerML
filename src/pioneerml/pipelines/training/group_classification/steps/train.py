...
def _split_dataset_to_graphs(dataset: GroupClassifierDataset) -> list[Data]:
    data = dataset.data
...
def train_group_classifier(
    ...
    step_config = resolve_step_config(pipeline_config, "train")
    ...
    if int(model_cfg["hidden"]) % int(model_cfg["heads"]) != 0:
        raise ValueError("hidden must be divisible by heads for GroupClassifier.")
    ...
    if not graphs:
        raise RuntimeError("No non-empty graphs found in dataset.")
