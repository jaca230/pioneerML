...
class EndpointRegressorTrainStep(BaseTrainingStep):
    step_key = "train"
    ...
    def execute(self) -> GraphLightningModule:
        ...
        if not train_provider.include_targets or not val_provider.include_targets:
            raise RuntimeError("EndpointRegressionGraphLoader must run in train mode for training/validation.")
...
@step(name="train_endpoint_regressor", output_materializers=GraphLightningModuleMaterializer)
def train_endpoint_regressor_step(
    dataset: BatchBundle,
    pipeline_config: dict | None = None,
    hpo_params: dict | None = None,
) -> Any:
    return EndpointRegressorTrainStep(dataset=dataset, pipeline_config=pipeline_config, hpo_params=hpo_params).execute()
