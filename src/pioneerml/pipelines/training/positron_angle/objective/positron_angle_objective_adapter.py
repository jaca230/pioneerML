...
from pioneerml.common.models.regressors import PositronAngleModel
...
from pioneerml.common.pipeline.services.training.utils import GraphLightningModule, QuantileAngularLoss


class PositronAngleObjectiveAdapter(BaseObjectiveAdapter):
    @staticmethod
    ...
    def suggest_model_params(self, *, trial: optuna.Trial, model_search_cfg: dict | None = None) -> dict:
        ...
        return {
            ...
            "group_prob_dimension": int(PositronAngleGraphLoader.NUM_CLASSES),
            "splitter_prob_dimension": int(PositronAngleGraphLoader.NUM_CLASSES),
            ...
            "pion_stop_pred_dimension": int(PositronAngleGraphLoader.PION_STOP_DIM),
            "hidden": int(hidden),
            "heads": int(heads),
    ...
    def default_model_cfg(self, model_cfg: dict | None = None) -> dict:
        ...
        cfg.setdefault("in_channels", int(PositronAngleGraphLoader.NODE_FEATURE_DIM))
        cfg.setdefault("group_prob_dimension", int(PositronAngleGraphLoader.NUM_CLASSES))
        cfg.setdefault("splitter_prob_dimension", int(PositronAngleGraphLoader.NUM_CLASSES))
        cfg.setdefault("endpoint_pred_dimension", int(PositronAngleGraphLoader.ENDPOINT_DIM))
        cfg.setdefault("event_affinity_dimension", int(PositronAngleGraphLoader.EVENT_AFFINITY_DIM))
        cfg.setdefault("pion_stop_pred_dimension", int(PositronAngleGraphLoader.PION_STOP_DIM))
        cfg.setdefault("hidden", 192)
        cfg.setdefault("heads", 4)
        cfg.setdefault("layers", 3)
        cfg.setdefault("dropout", 0.1)
        cfg.setdefault("output_dim", int(PositronAngleGraphLoader.TARGET_DIM))
        return cfg
    ...
    def build_module(self, *, model, train_cfg: dict):
        ...
        return GraphLightningModule(
            model,
            task="regression",
            loss_fn=QuantileAngularLoss(
                num_outputs=3,
                quantiles=(0.16, 0.50, 0.84),
                pinball_weight=float(loss_cfg.get("pinball_weight", 0.0)),
                angular_weight=float(loss_cfg.get("angular_weight", 1.0)),
                unit_norm_weight=float(loss_cfg.get("unit_norm_weight", 0.0)),
                normalize_target=bool(loss_cfg.get("normalize_target", False)),
                clamp_dot=bool(loss_cfg.get("clamp_dot", False)),
            ),
