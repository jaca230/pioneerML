...
from pioneerml.common.zenml.materializers import EndpointRegressorEventDatasetMaterializer
...
def load_endpoint_regressor_event_dataset_local(
    parquet_paths,
...
) -> EndpointRegressorEventDataset:
    adapter = pml.adapters.input.graph.EndpointRegressorEventInputAdapter()
    ...
    y = _DATASET_BUILDER.arrow_to_torch(targets.y, shape=(num_groups, 6), dtype=torch.float32)
    data.y = y
    return EndpointRegressorEventDataset(data=data, targets=y)


@step(enable_cache=False, output_materializers=EndpointRegressorEventDatasetMaterializer)
def load_endpoint_regressor_event_dataset(
    ...
    return load_endpoint_regressor_event_dataset_local(
        parquet_paths,
