...
class PositronAngleSavePredictionsService(
    PositronAngleInferenceServiceBase,
    ...
    def default_config(self) -> dict:
        ...
        cfg["output_dir"] = "data/positron_angle"
    ...
    def execute(
        ...
        *,
        ...
        out_dir = self.ensure_output_dir(output_dir, str(cfg.get("output_dir", "data/positron_angle")))
        ...
        validated_event_splitter_files = [
            str(Path(p).expanduser().resolve()) for p in inference_outputs.get("validated_event_splitter_files", [])
        ]
        validated_pion_stop_files = [
            str(Path(p).expanduser().resolve()) for p in inference_outputs.get("validated_pion_stop_files", [])
        ]
            ...
            if validated_files:
                ...
                for src_file, n_rows in zip(validated_files, row_counts, strict=True):
                    ...
                    table = self.build_prediction_table(
                        ...
                        num_rows=n_rows,
                        source_parquet_path=str(src_file),
                    )
                ...
                pred_path = Path(output_path) if output_path else (out_dir / "positron_angle_preds.parquet")
                self.atomic_write_table(table=table, dst_path=pred_path)
                per_file_output_paths.append(str(pred_path))
                if write_timestamped:
                    p = out_dir / f"positron_angle_preds_{ts}.parquet"
                    self.atomic_write_table(table=table, dst_path=p)
                    per_file_timestamped_paths.append(str(p))
        ...
        metrics = {
            ...
            "validated_event_splitter_files": validated_event_splitter_files,
            "validated_pion_stop_files": validated_pion_stop_files,
            "loss": None,
            "mae": None,
        }
