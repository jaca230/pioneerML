...
class EventSplitterHPOService(EventSplitterServiceBase, BaseHPOService):
    step_key = "hpo"
    ...
    def objective_context(self) -> str:
        return "tune_event_splitter"

    def study_name_default(self) -> str:
        return "event_splitter_hpo"
    ...
    def build_hpo_loaders(self, *, cfg: dict, batch_size: int):
        ...
        if not train_provider.include_targets or not val_provider.include_targets:
            raise RuntimeError("EventSplitterGraphLoader must run in train mode for HPO.")
