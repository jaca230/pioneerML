"""Event-level endpoint regressor for per-time-group start/end coordinates."""
...
class EndpointsRegressorEvent(nn.Module):
    """Predicts `[start_x, start_y, start_z, end_x, end_y, end_z]` per time group."""
    ...
    def forward_tensors(
        ...
        x = self.input_proj(torch.cat([raw_x, expanded_group_probs, splitter_probs], dim=1))
        ...
        pool_x = x.new_zeros((num_groups, feat_dim))
        pool_y = x.new_zeros((num_groups, feat_dim))
        pool_all = x.new_zeros((num_groups, feat_dim))
        counts_x = x.new_zeros((num_groups, 1))
        counts_y = x.new_zeros((num_groups, 1))
        counts_all = x.new_zeros((num_groups, 1))

        group_energy = x.new_zeros((num_groups, 1))
        one_vec = x.new_ones((x.size(0), 1))
        counts_all.index_add_(0, group_ids, one_vec)
        pool_all.index_add_(0, group_ids, x)
        edep = raw_x[:, 2].to(x.dtype).unsqueeze(1)
        group_energy.index_add_(0, group_ids, edep)
        ...
        if bool(mask_x.any().item()):
            gid_x = group_ids[mask_x]
            x_x = x[mask_x]
            pool_x.index_add_(0, gid_x, x_x)
            counts_x.index_add_(0, gid_x, x_x.new_ones((x_x.size(0), 1)))
        if bool(mask_y.any().item()):
            gid_y = group_ids[mask_y]
            x_y = x[mask_y]
            pool_y.index_add_(0, gid_y, x_y)
            counts_y.index_add_(0, gid_y, x_y.new_ones((x_y.size(0), 1)))
        ...
        out = torch.cat([pool_x, pool_y, pool_all, group_probs, group_energy, has_x, has_y], dim=1)
    ...
    def export_torchscript(
        ...
        *,
        ...
        class _Scriptable(nn.Module):
            def __init__(self, model: EndpointsRegressorEvent):
                super().__init__()
                self.model = model
...
EndpointRegressorEvent = EndpointsRegressorEvent
OrthogonalEndpointRegressorEvent = EndpointsRegressorEvent
