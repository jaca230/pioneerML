...
class PionStopSavePredictionsService(
    PionStopInferenceServiceBase,
    ...
    def execute(
        ...
        *,
        ...
        out_dir = self.ensure_output_dir(output_dir, str(cfg.get("output_dir", "data/pion_stop")))
        ...
        if streamed:
            ...
            if validated_files:
                ...
                pred_path = Path(output_path) if output_path else (out_dir / "pion_stop_preds.parquet")
                self.atomic_write_table(table=table, dst_path=pred_path)
                per_file_output_paths.append(str(pred_path))
                if write_timestamped:
                    p = out_dir / f"pion_stop_preds_{ts}.parquet"
                    self.atomic_write_table(table=table, dst_path=p)
                    per_file_timestamped_paths.append(str(p))
