...
class PositronAngleGraphLoaderFactory(BaseGraphLoaderFactory):
    def __init__(
        self,
        *,
        ...
        self.event_splitter_parquet_paths = (
            [str(p) for p in event_splitter_parquet_paths] if event_splitter_parquet_paths else None
        )
        self.pion_stop_parquet_paths = [str(p) for p in pion_stop_parquet_paths] if pion_stop_parquet_paths else None
    ...
    def build_loader(self, *, loader_params: dict) -> PositronAngleGraphLoader:
        cfg = dict(loader_params or {})
        return PositronAngleGraphLoader(
            ...
            endpoint_parquet_paths=(
                list(self.endpoint_parquet_paths) if self.endpoint_parquet_paths is not None else None
            ),
            event_splitter_parquet_paths=(
                list(self.event_splitter_parquet_paths)
                if self.event_splitter_parquet_paths is not None
                else None
            ),
            pion_stop_parquet_paths=(
                list(self.pion_stop_parquet_paths) if self.pion_stop_parquet_paths is not None else None
            ),
            ...
            use_event_splitter_affinity=bool(cfg.get("use_event_splitter_affinity", True)),
            use_pion_stop_preds=bool(cfg.get("use_pion_stop_preds", True)),
            training_relevant_only=bool(cfg.get("training_relevant_only", True)),
