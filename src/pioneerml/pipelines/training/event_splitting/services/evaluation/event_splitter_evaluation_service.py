...
class EventSplitterEvaluationService(EventSplitterServiceBase, BaseEvaluationService):
    step_key = "evaluate"
    ...
    def _evaluate_from_loader(self, *, loader, threshold: float, plot_config: dict | None) -> dict:
        ...
        for cls_idx in range(num_classes):
            ...
            confusion_metrics.append(
                {"tp": tp / total, "fp": fp / total, "fn": fn / total}
                if total > 0
    ...
    def execute(self) -> dict:
        ...
        if not provider.include_targets:
            raise RuntimeError("EventSplitterGraphLoader must run in train mode for evaluation.")
