...
class EndpointRegressorObjectiveAdapter(BaseObjectiveAdapter):
    ...
    def suggest_model_params(self, *, trial: optuna.Trial, model_search_cfg: dict | None = None) -> dict:
        ...
        return {
            "in_channels": int(EndpointRegressorGraphLoader.NODE_FEATURE_DIM),
            "group_prob_dimension": int(EndpointRegressorGraphLoader.NUM_CLASSES),
            "splitter_prob_dimension": int(EndpointRegressorGraphLoader.NUM_CLASSES),
            "hidden": int(hidden),
            "heads": int(heads),
    ...
    def default_model_cfg(self, model_cfg: dict | None = None) -> dict:
        ...
        cfg.setdefault("in_channels", int(EndpointRegressorGraphLoader.NODE_FEATURE_DIM))
        cfg.setdefault("group_prob_dimension", int(EndpointRegressorGraphLoader.NUM_CLASSES))
        cfg.setdefault("splitter_prob_dimension", int(EndpointRegressorGraphLoader.NUM_CLASSES))
        cfg.setdefault("hidden", 192)
        cfg.setdefault("heads", 4)
        cfg.setdefault("layers", 3)
        cfg.setdefault("dropout", 0.1)
        cfg.setdefault("output_dim", int(EndpointRegressorGraphLoader.TARGET_DIM))
        return cfg
